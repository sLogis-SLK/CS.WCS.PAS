...
public class frmPAS00063 : Form
{
  ...
  private RButton 실적전송취소버튼;
  private RButton 배치반영취소버튼;
  private RButton 실적작성취소버튼;
  ...
  private void frmPAS00063_Load(object sender, EventArgs e)
  {
  ...
  private void 실적전송취소버튼_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      else
      {
        ...
        if (Common.QuestionMessageBox($"배치번호 : {oValue1}\r\n\r\n선택한 배치의 실적을 취소 하시겠습니까?") == DialogResult.Yes)
        {
          DbProvider.Excute(Common.ConnectionString(DB연결구분.시스템), "usp_PAS_실적반영취소_Set", DbProvider.GetParameter("@배치번호", (object) oValue1, ParameterDirection.Input));
          ...
          Common.OkMessageBox("실적 작성 취소가 완료되었습니다.");
        }
        else
  ...
  private void 실적작성취소버튼_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      else
      {
        ...
        if (Common.QuestionMessageBox($"배치번호 : {oValue}\r\n\r\n선택한 배치의 작성된 실적을 취소 하시겠습니까?") != DialogResult.Yes)
        {
          Common.ErrorMessageBox("작업을 취소합니다!!");
        }
        else
        {
          DbProvider.Excute(Common.ConnectionString(), "usp_분류_실적작성취소_Set", DbProvider.GetParameter("@배치번호", (object) oValue, ParameterDirection.Input));
          Common.OkMessageBox("실적 작성이 취소되었습니다.");
        }
      }
    }
    catch (Exception ex)
  ...
  private void 배치반영취소버튼_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      else
      {
        string oValue1 = selectedRows[0].Cells["배치번호"].Value.ToString();
        string oValue2 = selectedRows[0].Cells["분류번호"].Value.ToString();
        if (Common.QuestionMessageBox($"배치번호 : {oValue1}\r\n\r\n선택한 배치의 패킹실적을 취소 하시겠습니까?") == DialogResult.Yes)
        {
          DbProvider.Excute(Common.ConnectionString(DB연결구분.시스템), "usp_PAS_배치반영취소_Set", DbProvider.GetParameter("@배치번호", (object) oValue1, ParameterDirection.Input));
          DbProvider.Excute(Common.ConnectionString(), "usp_분류_배치상태변경_Set", DbProvider.GetParameter("@장비명", (object) Common.Setting.NAME, ParameterDirection.Input), DbProvider.GetParameter("@분류번호", (object) oValue2, ParameterDirection.Input), DbProvider.GetParameter("@배치번호", (object) oValue1, ParameterDirection.Input), DbProvider.GetParameter("@배치상태", (object) "실적반영", ParameterDirection.Input));
          Common.OkMessageBox("패킹실적 작성 취소가 완료되었습니다.");
        }
        else
          Common.ErrorMessageBox("작업을 취소합니다!!");
      }
  ...
  private void InitializeComponent()
  {
    ...
    this.실적작성취소버튼 = new RButton();
    this.배치반영취소버튼 = new RButton();
    this.실적전송취소버튼 = new RButton();
    ...
    this.rPanel1.Controls.Add((System.Windows.Forms.Control) this.실적작성취소버튼);
    this.rPanel1.Controls.Add((System.Windows.Forms.Control) this.배치반영취소버튼);
    this.rPanel1.Controls.Add((System.Windows.Forms.Control) this.실적전송취소버튼);
    ...
    this.실적작성취소버튼.Anchor = AnchorStyles.Top | AnchorStyles.Right;
    this.실적작성취소버튼.ButtonState = RButtonState.None;
    this.실적작성취소버튼.Font = new Font("굴림", 8.25f, FontStyle.Bold, GraphicsUnit.Point, (byte) 129);
    this.실적작성취소버튼.ForeColor = Color.DimGray;
    this.실적작성취소버튼.Image = (Image) pas.mgp.Properties.Resources._1397196465_app_cancel;
    this.실적작성취소버튼.Location = new Point(560, 11);
    this.실적작성취소버튼.Name = "실적작성취소버튼";
    this.실적작성취소버튼.Size = new Size(140, 23);
    this.실적작성취소버튼.TabIndex = 4;
    this.실적작성취소버튼.Text = "PAS 실적작성 취소";
    this.실적작성취소버튼.UseVisualStyleBackColor = true;
    this.실적작성취소버튼.Click += new EventHandler(this.실적작성취소버튼_Click);
    this.배치반영취소버튼.Anchor = AnchorStyles.Top | AnchorStyles.Right;
    this.배치반영취소버튼.ButtonState = RButtonState.None;
    this.배치반영취소버튼.Font = new Font("굴림", 8.25f, FontStyle.Bold, GraphicsUnit.Point, (byte) 129);
    this.배치반영취소버튼.ForeColor = Color.DimGray;
    this.배치반영취소버튼.Image = (Image) pas.mgp.Properties.Resources._1395148855_recv_cancel;
    this.배치반영취소버튼.Location = new Point(842, 11);
    this.배치반영취소버튼.Name = "배치반영취소버튼";
    this.배치반영취소버튼.Size = new Size(140, 23);
    this.배치반영취소버튼.TabIndex = 3;
    this.배치반영취소버튼.Text = "WMS 패킹반영 취소";
    this.배치반영취소버튼.UseVisualStyleBackColor = true;
    this.배치반영취소버튼.Click += new EventHandler(this.배치반영취소버튼_Click);
    this.실적전송취소버튼.Anchor = AnchorStyles.Top | AnchorStyles.Right;
    this.실적전송취소버튼.ButtonState = RButtonState.None;
    this.실적전송취소버튼.Font = new Font("굴림", 8.25f, FontStyle.Bold, GraphicsUnit.Point, (byte) 129);
    this.실적전송취소버튼.ForeColor = Color.DimGray;
    this.실적전송취소버튼.Image = (Image) pas.mgp.Properties.Resources._1395148855_recv_cancel;
    this.실적전송취소버튼.Location = new Point(701, 11);
    this.실적전송취소버튼.Name = "실적전송취소버튼";
    this.실적전송취소버튼.Size = new Size(140, 23);
    this.실적전송취소버튼.TabIndex = 2;
    this.실적전송취소버튼.Text = "WMS 실적반영 취소";
    this.실적전송취소버튼.UseVisualStyleBackColor = true;
    this.실적전송취소버튼.Click += new EventHandler(this.실적전송취소버튼_Click);
    ...
    this.조회버튼.Location = new Point(469, 11);
    this.조회버튼.Name = "조회버튼";
    ...
    this.Name = nameof (frmPAS00063);
    this.Text = "frmPAS00062";
    this.Load += new EventHandler(this.frmPAS00063_Load);
