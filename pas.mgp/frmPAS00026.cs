...
public class frmPAS00026 : Form
{
  ...
  private void frmPAS00026_Load(object sender, EventArgs e)
  {
    this.분류_작업요약_조회(조회구분자.가조회);
    this.출하_박스별패킹대상_조회(string.Empty, string.Empty, 조회구분자.가조회);
  }
  ...
  private void 출하_박스별패킹대상_조회(string s분류번호, string s배치번호, 조회구분자 e)
  {
    this.checkBox1.Checked = false;
    DbProvider.Select(Common.ConnectionString(), this.m_출하_박스별패킹대상Table, DbProvider.GetParameter("@분류번호", (object) s분류번호, ParameterDirection.Input), DbProvider.GetParameter("@장비명", (object) Common.Setting.NAME, ParameterDirection.Input), DbProvider.GetParameter("@배치번호", (object) s배치번호, ParameterDirection.Input), DbProvider.GetParameter("@조회구분자", (object) (int) e, ParameterDirection.Input));
    this.rGrid2.DataSource2 = (object) this.m_출하_박스별패킹대상Table;
    this.rGrid2.SetSummaries(new DataGridViewSummary("선택", SummaryType.카운트));
  ...
  private void 조회버튼_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      this.출하_박스별패킹대상_조회(empty1, empty2, 조회구분자.실조회);
    }
    catch (Exception ex)
    {
  ...
  private void rGrid1_MouseUp(object sender, MouseEventArgs e)
  {
    try
    {
      ...
      string empty4 = string.Empty;
      string empty5 = string.Empty;
      this.출하_박스별패킹대상_조회(this.rGrid1.Rows[hitTestInfo.RowIndex].Cells["분류번호"].Value.ToString(), this.rGrid1.Rows[hitTestInfo.RowIndex].Cells["배치번호"].Value.ToString(), 조회구분자.실조회);
    }
    catch (Exception ex)
    {
      Common.ErrorMessageBox(ex.Message);
  ...
  public void Print()
  {
    ...
    if (dlgPaS00026.ShowDialog() == DialogResult.OK)
    {
      ...
      Common.OkMessageBox("미리보기를 지원하지 않습니다.");
      Cursor.Current = Cursors.WaitCursor;
      try
      {
        this.rGrid2.EndEdit();
        this.m_출하_박스별패킹대상Table.AcceptChanges();
        DataTable oDataTable = new DataTable("usp_기준_거래명세서용_마스터_Get");
        DbProvider.Select(Common.ConnectionString(), oDataTable, (SqlParameter[]) null);
        ...
        else
        {
          ...
          foreach (DataGridViewRow row in (IEnumerable) this.rGrid2.Rows)
          {
            if (row.Cells["선택"].Value.ToString() == bool.TrueString)
            {
              ...
              DataRow[] dataRowArray = oDataTable.Select($"슈트번호 = '{s슈트번호}'");
  ...
  private void InitializeComponent()
  {
    ...
    this.Name = nameof (frmPAS00026);
    this.Text = "frmPAS00021";
    this.Load += new EventHandler(this.frmPAS00026_Load);
