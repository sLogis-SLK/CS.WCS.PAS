...
public class frmPAS00022 : Form
{
  private DataTable m_분류_작업요약_배치그룹별Table = new DataTable("usp_분류_작업요약_배치그룹별_Get");
  private DataTable m_분류_미출고내역_상품별Table = new DataTable("usp_분류_미출고내역_상품별_Get");
  ...
  private void frmPAS00022_Load(object sender, EventArgs e)
  {
    this.분류_작업요약_조회(조회구분자.가조회);
    this.분류_미출고내역_상품별_조회(string.Empty, string.Empty, 조회구분자.가조회);
  }
  ...
  private void 조회버튼_Click(object sender, EventArgs e)
  {
    try
    {
      ...
      this.분류_미출고내역_상품별_조회(empty1, empty2, 조회구분자.실조회);
    }
    catch (Exception ex)
    {
  ...
  private void rGrid1_MouseUp(object sender, MouseEventArgs e)
  {
    try
    {
      ...
      string empty2 = string.Empty;
      string empty3 = string.Empty;
      this.분류_미출고내역_상품별_조회(this.rGrid1.Rows[hitTestInfo.RowIndex].Cells["분류번호"].Value.ToString(), this.rGrid1.Rows[hitTestInfo.RowIndex].Cells["배치번호"].Value.ToString(), 조회구분자.실조회);
    }
    catch (Exception ex)
    {
      Common.ErrorMessageBox(ex.Message);
  ...
  public void Save()
  {
    ...
    try
    {
      ...
      else
      {
        ...
        DataTable oDataTable = new DataTable("usp_분류_미출고내역_상품별_Get");
        DbProvider.Select(Common.ConnectionString(), oDataTable, DbProvider.GetParameter("@분류번호", (object) oValue1, ParameterDirection.Input), DbProvider.GetParameter("@장비명", (object) Common.Setting.NAME, ParameterDirection.Input), DbProvider.GetParameter("@배치번호", (object) oValue2, ParameterDirection.Input), DbProvider.GetParameter("@조회구분자", (object) 1, ParameterDirection.Input));
  ...
  public void Print()
  {
    ...
    try
    {
      ...
      else
      {
        string oValue1 = this.rGrid1.SelectedRows[0].Cells["분류번호"].Value.ToString();
        string oValue2 = this.rGrid1.SelectedRows[0].Cells["배치번호"].Value.ToString();
        DataTable dataTable = new DataTable("usp_분류_미출고내역_상품별_Get");
        DbProvider.Select(Common.ConnectionString(), dataTable, DbProvider.GetParameter("@분류번호", (object) oValue1, ParameterDirection.Input), DbProvider.GetParameter("@장비명", (object) Common.Setting.NAME, ParameterDirection.Input), DbProvider.GetParameter("@배치번호", (object) oValue2, ParameterDirection.Input), DbProvider.GetParameter("@조회구분자", (object) 1, ParameterDirection.Input));
        ...
        {
          string val = $"{dataTable.Rows[0]["브랜드코드"].ToString()}:{dataTable.Rows[0]["브랜드명"].ToString()}";
          미출고내역_상품별 미출고내역상품별 = new 미출고내역_상품별();
          미출고내역상품별.SetDataSource(dataTable);
          미출고내역상품별.SetParameterValue("로컬장비명", (object) Common.Setting.NAME);
          ...
          crystalReportViewer.ReportSource = (object) 미출고내역상품별;
  ...
  private void InitializeComponent()
  {
    ...
    this.Name = nameof (frmPAS00022);
    this.Text = "frmPAS00021";
    this.Load += new EventHandler(this.frmPAS00022_Load);
