...
public class frmPAS00027 : Form
{
  ...
  public frmPAS00027()
  {
    ...
    Common.RGrid_Initializing(this.rGrid4, false, false);
    this.rLabel1.Control = (System.Windows.Forms.Control) this.dateTimePicker1;
    this.rLabel5.Control = (System.Windows.Forms.Control) this.comboBox1;
  }

  private void frmPAS00027_Load(object sender, EventArgs e)
  {
    ...
    조회구분자 e)
  {
    ...
    foreach (DataRow row in (InternalDataCollectionBase) this.m_분류_박스바코드재발행_슈트별Table.Rows)
    {
      string str = row["박스번호"].ToString();
      if (!string.IsNullOrEmpty(str) && !this.comboBox1.Items.Contains((object) str))
  ...
  private void rGrid4_CellEndEdit(object sender, DataGridViewCellEventArgs e)
  {
    if (this.rGrid4.Rows.Count <= 0)
      return;
    try
    {
      this.rGrid4.EndEdit();
      string str = this.rGrid4.Rows[e.RowIndex].Cells["아이템코드"].Value.ToString();
      int num1 = Common.C2I((object) this.rGrid4.Rows[e.RowIndex].Cells["수량"].Value.ToString());
      int num2 = Common.C2I((object) this.rGrid4.Rows[e.RowIndex].Cells["조정"].Value.ToString());
      int num3 = Common.C2I((object) this.rGrid4.Rows[e.RowIndex].Cells["잔여"].Value.ToString());
      DataRow[] dataRowArray = this.m_분류_박스바코드재발행_슈트별상세Table.Select($"아이템코드='{str}'");
      if (dataRowArray == null || dataRowArray.Length <= 0)
        return;
      if (num1 - num2 < 0)
      {
        Common.ErrorMessageBox("조정수가 실적수보다 많을 수 없습니다.");
        dataRowArray[0]["조정"] = (object) (num1 - num3);
        dataRowArray[0]["잔여"] = (object) num3;
      }
      else
      {
        dataRowArray[0]["잔여"] = (object) (num1 - num2);
        this.m_분류_박스바코드재발행_슈트별상세Table.AcceptChanges();
      }
    }
    catch (Exception ex)
    {
  ...
  private void 이동버튼_Click(object sender, EventArgs e)
  {
    try
    {
      Cursor.Current = Cursors.WaitCursor;
      this.rGrid1.EndEdit();
      DataRow[] dataRowArray = this.m_분류_박스바코드재발행_슈트별상세Table.Select("수량 <> 잔여");
      if (dataRowArray == null || dataRowArray.Length <= 0)
      {
        Common.ErrorMessageBox("이동할 대상이 없습니다.");
      }
      else
      {
        string empty1 = string.Empty;
        ...
        {
          Common.ErrorMessageBox("배치번호를 선택하세요.");
        }
        else
        {
          string str1 = this.rGrid1.SelectedRows[0].Cells["배치번호"].Value.ToString();
          ...
          {
            string str2 = this.rGrid2.SelectedRows[0].Cells["슈트번호"].Value.ToString();
            if (this.rGrid3.SelectedRows == null || this.rGrid3.SelectedRows.Count <= 0)
            {
              Common.ErrorMessageBox("박스번호를 선택하세요.");
            }
            else
            {
              string str3 = this.rGrid3.SelectedRows[0].Cells["박스번호"].Value.ToString();
              if (string.IsNullOrEmpty(this.comboBox1.Text))
                Common.ErrorMessageBox("이동할 대상 박스가 없습니다.");
              ...
              else
              {
                ...
                {
                  dbProvider2.Initialize("usp_출하_상품이동_Set", "@배치번호", "@슈트번호", "@박스번호", "@대상박스번호", "@아이템코드", "@조정수");
                  foreach (DataRow dataRow in dataRowArray)
                    dbProvider2.Update((object) str1, (object) str2, (object) str3, (object) this.comboBox1.Text, dataRow["아이템코드"], dataRow["조정"]);
                  dbProvider2.Commit();
                }
                Common.OkMessageBox("상품이동 처리를 완료 하였습니다.");
              }
            }
          }
        }
  ...
  private void InitializeComponent()
  {
    ...
    this.panel2.Controls.Add((System.Windows.Forms.Control) this.comboBox1);
    this.panel2.Controls.Add((System.Windows.Forms.Control) this.rLabel5);
    this.panel2.Controls.Add((System.Windows.Forms.Control) this.이동버튼);
    this.panel2.Dock = DockStyle.Bottom;
    this.panel2.Location = new Point(0, 565);
    this.panel2.Name = "panel2";
    this.panel2.Size = new Size(1000, 45);
    this.panel2.TabIndex = 5;
    this.label1.AutoSize = true;
    this.label1.Font = new Font("굴림", 8.25f, FontStyle.Bold);
    this.label1.ForeColor = Color.DimGray;
    this.label1.Location = new Point(236, 16 /*0x10*/);
    this.label1.Name = "label1";
    this.label1.Size = new Size(17, 11);
    this.label1.TabIndex = 28;
    this.label1.Text = "로";
    this.comboBox1.DropDownStyle = ComboBoxStyle.DropDownList;
    this.comboBox1.FormattingEnabled = true;
    this.comboBox1.Location = new Point(130, 11);
    ...
    this.rLabel5.Location = new Point(12, 10);
    this.rLabel5.Name = "rLabel5";
    this.rLabel5.Size = new Size(120, 23);
    this.rLabel5.TabIndex = 26;
    this.rLabel5.Text = "이동할 박스번호";
    this.이동버튼.ButtonState = RButtonState.None;
    this.이동버튼.Font = new Font("굴림", 8.25f, FontStyle.Bold, GraphicsUnit.Point, (byte) 129);
    this.이동버튼.ForeColor = Color.DimGray;
    this.이동버튼.Image = (Image) pas.mgp.Properties.Resources.add_on;
    this.이동버튼.Location = new Point(259, 10);
    this.이동버튼.Name = "이동버튼";
    this.이동버튼.Size = new Size(90, 23);
    this.이동버튼.TabIndex = 25;
    this.이동버튼.Text = "이 동";
    this.이동버튼.UseVisualStyleBackColor = true;
    this.이동버튼.Click += new EventHandler(this.이동버튼_Click);
    ...
    this.panel3.Size = new Size(518, 370);
    ...
    this.rGrid2.Size = new Size(300, 358);
    ...
    this.rGrid3.Size = new Size(200, 358);
    ...
    this.splitter1.Size = new Size(3, 370);
    this.splitter1.TabIndex = 8;
    this.splitter1.TabStop = false;
    this.panel4.Controls.Add((System.Windows.Forms.Control) this.rGrid4);
    this.panel4.Dock = DockStyle.Fill;
    this.panel4.Location = new Point(521, 195);
    this.panel4.Name = "panel4";
    this.panel4.Size = new Size(479, 370);
    ...
    this.rGrid4.Size = new Size(467, 358);
    this.rGrid4.TabIndex = 9;
    this.rGrid4.CellEndEdit += new DataGridViewCellEventHandler(this.rGrid4_CellEndEdit);
    ...
    this.Name = nameof (frmPAS00027);
    this.Text = "frmPAS00021";
    this.Load += new EventHandler(this.frmPAS00027_Load);
